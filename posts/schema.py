...
from .enums import PostVisibilityEnums, GPSAccuracyEnums
...
class GPSAccuracyEnumsType(graphene.Enum):
    ONEHUNDREDKILOMETERS = GPSAccuracyEnums.ONEHUNDREDKILOMETERS
    TENKILOMETERS = GPSAccuracyEnums.TENKILOMETERS
    ONEKILOMETER = GPSAccuracyEnums.ONEKILOMETER
    ONEHUNDREDMETERS = GPSAccuracyEnums.ONEHUNDREDMETERS
    TENMETERS = GPSAccuracyEnums.TENMETERS
    ONEMETER = GPSAccuracyEnums.ONEMETER
...
class PostsQuery(graphene.AbstractType):
    ...
    post = graphene.Field(PostType, id=graphene.ID(
        required=True), description="Get one post based on given id")
    posts = graphene.List(
        PostType, description="Get all posts in the database that are PUBLIC")
    feed_posts = graphene.List(
        PostType, description="Gets all posts based on users followed by current user")
    posts_by_tag = graphene.List(PostType, tags=graphene.List(
        graphene.String, required=True), description="Gets all posts based on given tags")
    posts_by_location = graphene.List(PostType, description="Gets all posts based on given tags",
        latitude=graphene.Decimal(required=True, description="GPS Coordinates: Latitude"),
        longitude=graphene.Decimal(required=True, description="GPS Coordinates: longitude"),
        precision =GPSAccuracyEnumsType(required=True)
    )
    ...
    def resolve_post(self, info, id):
        if not info.context.user.is_authenticated:
            ...
            if (post.author.visibility == ProfileVisibilityEnums.PRIVATE) and not (UserFollows.objects.filter(user_profile=current_user_profile, following_user_profile=post.author).exists()) and not (current_user_profile == post.author):
                raise GraphQLError(
                    'You must be following post author to get private post!')
            else:
                return Post.objects.get(post_id=id)

    def resolve_posts(self, info):
        public_users = Profile.objects.filter(
            visibility=ProfileVisibilityEnums.PUBLIC)
    ...
    def resolve_feed_posts(self, info):
        if not info.context.user.is_authenticated:
            ...
            following = UserFollows.objects.filter(
                user_profile=current_user_profile).values('following_user_profile')
            criterion1 = Q(author__in=following,
                           visibility=PostVisibilityEnums.ACTIVE)
            criterion2 = Q(author=current_user_profile,
                           visibility=PostVisibilityEnums.ACTIVE)
    ...
    def resolve_posts_by_tag(self, info, tags=[]):
        if not info.context.user.is_authenticated:
            ...
            print(tagobjects)
            criterion1 = Q(author__in=public_users,
                           visibility=PostVisibilityEnums.ACTIVE, tags__in=tagobjects)
            criterion2 = Q(
                author__in=following, visibility=PostVisibilityEnums.ACTIVE, tags__in=tagobjects)
            return Post.objects.filter(criterion1 | criterion2).order_by('-date_created')

    def resolve_posts_by_location(self, info, latitude, longitude, precision):
        if not info.context.user.is_authenticated:
            raise GraphQLError('You must be logged to get post by location!')
        else:
            current_user_profile = Profile.objects.get(user=info.context.user)

            radius = Decimal(1 / precision)
            
            if precision == GPSAccuracyEnumsType.ONEHUNDREDKILOMETERS:
                radius.quantize(Decimal('1'))
            elif precision == GPSAccuracyEnumsType.TENKILOMETERS:
                radius.quantize(Decimal('1.0'))
            elif precision == GPSAccuracyEnumsType.ONEKILOMETER:
                radius.quantize(Decimal('1.00'))
            elif precision == GPSAccuracyEnumsType.ONEHUNDREDMETERS:
                radius.quantize(Decimal('1.000'))
            elif precision == GPSAccuracyEnumsType.TENMETERS:
                radius.quantize(Decimal('1.0000'))
            elif precision == GPSAccuracyEnumsType.ONEKILOMETER:
                radius.quantize(Decimal('1.00000'))

            lat_upper = latitude + radius
            lat_lower = latitude - radius

            long_upper = longitude + radius
            long_lower = longitude - radius

            latitude_radius = Q(gps_latitude__range=(lat_lower, lat_upper))
            longitude_radius = Q(gps_longitude__range=(long_lower, long_upper))
...
class CreatePost(graphene.Mutation):
    ...
    class Arguments:
        caption = graphene.String(
            default_value="", description="An (optional) textual description.")
        gps_longitude = graphene.Decimal(
            default_value=0.0, description="GPS Coordinates: Longitude")
        gps_latitude = graphene.Decimal(
            default_value=0.0, description="GPS Coordinates: Latitude")
        tagged_users = graphene.List(
            graphene.String, description="List of usernames of tagged users in post.")
        tags = graphene.List(
            graphene.String, description="List of tags asscoiated with the post.")
        image = graphene.String(
            required=True, description="Image media for post.")
        channel = graphene.String(
            default_value='', description="Channel name for post.")

    post = graphene.Field(
        PostType, description="Returns the new post that was created successfully.")
    success = graphene.Boolean(
        default_value=False, description="Returns whether the post was created successfully.")
    ...
    def mutate(self, info, image, caption, channel, gps_longitude, gps_latitude, tagged_users=[], tags=[]):
        if not info.context.user.is_authenticated:
            ...
            current_user_profile = Profile.objects.get(user=info.context.user)
            post = Post(author=current_user_profile,
                        image=info.context.FILES[image], caption=caption, gps_longitude=gps_longitude, gps_latitude=gps_latitude)
            post.save()

            for user in tagged_users:
                post.tagged_users.add(Profile.objects.get(
            ...
            if channel != '':
                if not Channel.objects.filter(name=channel).exists():
                    raise GraphQLError(
                        'Channel does not exist. Provide existing Channel')
...
class PostUpvote(graphene.Mutation):
    class Arguments:
        post_id = graphene.ID(
            required=True, description="Unique ID for post to be upvoted")
        modifier = ModifierEnumsType(
            required=True, description="Add or remove")

    post_upvotes = graphene.Int(description="Number of upvotes for post")
    success = graphene.Boolean(
        default_value=False, description="Returns whether the post was upvoted successfully.")
    ...
    def mutate(self, info, post_id, modifier):
        ...
        if not info.context.user.is_authenticated:
            ...
            if (post.author.visibility == ProfileVisibilityEnums.PRIVATE) and not (UserFollows.objects.filter(user_profile=current_user_profile, following_user_profile=post.author).exists()) and not (current_user_profile == post.author):
                raise GraphQLError(
                    'You must be following post author to upvote private post!')
            else:
...
class PostComment(graphene.Mutation):
    class Arguments:
        post_id = graphene.ID(
            required=True, description="Unique ID for post to be commented")
        text = graphene.String(
            required=True, description="Comment text for post")

    success = graphene.Boolean(
        default_value=False, description="Returns whether the post was commented successfully.")
    ...
    def mutate(self, info, post_id, text):
        ...
        if not info.context.user.is_authenticated:
            ...
            if (post.author.visibility == ProfileVisibilityEnums.PRIVATE) and not (UserFollows.objects.filter(user_profile=current_user_profile, following_user_profile=post.author).exists()) and not (current_user_profile == post.author):
                raise GraphQLError(
                    'You must be following post author to comment on private post!')
            else:
                comment = Comment(author=current_user_profile,
                                  post_id=post, comment_content=text)
                comment.save()
...
class PostCommentReply(graphene.Mutation):
    class Arguments:
        ...
        text = graphene.String(
            required=True, description="Comment text for post")
        reply_to_id = graphene.ID(
            required=True, description="Unique ID for comment reply")
        ...
        if not info.context.user.is_authenticated:
            ...
            if (post.author.visibility == ProfileVisibilityEnums.PRIVATE) and not (UserFollows.objects.filter(user_profile=current_user_profile, following_user_profile=post.author).exists()) and not (current_user_profile == post.author):
                ...
                comment = Comment(author=current_user_profile, post_id=post, comment_content=text,
                                  reply_to_comment=Comment.objects.get(comment_id=reply_to_id))
                comment.save()
...
class PostIncrementViewCounter(graphene.Mutation):
    ...
    def mutate(self, info, post_id):
        ...
        if (post.author.visibility == ProfileVisibilityEnums.PRIVATE) and not (UserFollows.objects.filter(user_profile=current_user_profile, following_user_profile=post.author).exists()) and not (current_user_profile == post.author):
            raise GraphQLError(
                'You must be following post author to view private post!')
        else:
...
class DeletePost(graphene.Mutation):
    class Arguments:
        post_id = graphene.ID(
            required=True, description="Unique ID for post to be deleted")

    success = graphene.Boolean(
        default_value=False, description="Returns whether the post was deleted successfully.")
    ...
    def mutate(self, info, post_id):
        ...
        if not info.context.user.is_authenticated:
            ...
            if (post.author != current_user_profile):
                raise GraphQLError(
                    'You must be post author to delete on post!')
            else:
                post.delete()
...
class EditPostTaggedUsers(graphene.Mutation):
    class Arguments:
        post_id = graphene.ID(
            required=True, description="Unique ID for post to be modified")
        tagged_users = graphene.List(
            graphene.String, required=True, description="List of usernames of to be/removed tagged users in post.")
        modifier = ModifierEnumsType(
            required=True, description="Add or remove")

    success = graphene.Boolean(
        default_value=False, description="Returns whether the post was edited successfully.")
    ...
    def mutate(self, info, post_id, modifier, tagged_users):
        ...
        if not info.context.user.is_authenticated:
            ...
            if (post.author != current_user_profile):
                ...
                if modifier == ModifierEnumsType.REMOVE:
                    for user in tagged_users:
                        post.tagged_users.remove(Profile.objects.get(
                            user=User.objects.get(username=user)))
                        post.save()
...
class EditPostVisibility(graphene.Mutation):
    class Arguments:
        post_id = graphene.ID(
            required=True, description="Unique ID for post to be modified")
        modifier = PostVisibilityType(
            required=True, description="Hidden or Visibile")
...
class EditPostTags(graphene.Mutation):
    class Arguments:
        ...
        tags = graphene.List(graphene.String, required=True,
                             description="List of tags of to be/removed in post.")
        modifier = ModifierEnumsType(
            required=True, description="Add or remove")
